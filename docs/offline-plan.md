...
setlists (key: setlistId)
  value: { setlist: SetlistRow, songs: SetlistSongItem[], cachedAt: number, schemaVersion: 1 }
...
- 結合表 `setlist_songs` は IDB に持ち込まず、setlist 内 `songs: SetlistSongItem[]` で並び順 + ナビゲーションに必要な軽量セクション情報を保持
- 曲本体（フル `SongRow` + `SectionRow[]`）は `songs` ストアに集約。Perform view の SWR は曲ストア側を読む
